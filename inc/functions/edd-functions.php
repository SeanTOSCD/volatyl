...
// Number of downloads to show on download archives
function vol_download_archives_count( $query ) {
	if ( ( $query->is_post_type_archive( 'download' ) || $query->is_tax( 'download_category' ) || $query->is_tax( 'download_tag' ) ) && ! is_admin() ) {
		$query->set( 'posts_per_page', 8 );
	}
	return $query;
}
add_filter( 'pre_get_posts', 'vol_download_archives_count' );
